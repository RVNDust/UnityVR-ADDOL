...
public class NetworkManagerMultiConfig : NetworkManager {
   ...
   {
       // Server call
       Debug.Log("isDeviceActive:" + UnityEngine.XR.XRSettings.isDeviceActive);
       GameObject prefabToSpawn;

       switch (AppConfig.Inst.DeviceUsed.ToLower())
       {
           case "htc":
               prefabToSpawn = PlayerVR;
               break;
           ...
           default: // "auto" and others
               prefabToSpawn = UnityEngine.XR.XRSettings.isDeviceActive ? PlayerVR : PlayerThirdPerson;
               break;
       }
       var player = (GameObject)GameObject.Instantiate(prefabToSpawn, Vector3.zero, Quaternion.identity);
       if(autoCreatePlayer) NetworkServer.AddPlayerForConnection(conn, player, playerControllerId);
